...
- 关键帧采样（默认每 2 秒一帧，供规则引擎降级路径使用）

### Step 2: Token 预估（⚠️ 多模态路径必须告知用户）

多模态检测会上传完整视频文件到 Ark Files API，然后通过 `video_url` 发送给豆包模型进行音画联合理解：

- **预估**: 视频上传不消耗 tokens，API 调用 tokens 取决于视频时长和分辨率
- **告知用户**: 提示"实际以 API 返回的 usage 为准"
...
**主路径 — 原生视频理解（Ark API）:**
- 将完整视频（含音频）通过 Files API 上传获取预签名下载链接
- 以 `video_url` 形式发送给豆包多模态模型进行音画联合分析
- 模型综合画面内容、音频特征、音画配合等维度识别高光片段
...
4. **降级说明（如有）**: 明确告知用户哪些环节发生了降级，从什么路径降级到什么路径，以及降级原因
5. **输出视频**: 集锦视频文件路径或下载链接
6. **JSON 导出**: 结构化片段数据
...
- **长视频处理**: 超过 30 分钟的视频建议先分片处理，避免单次多模态调用上下文溢出。多模态路径上传完整视频文件，受 Files API 文件大小限制
...
- **关键帧采样为空**: 视频无法解码或帧数过少时，仅影响规则引擎降级路径，多模态主路径使用完整视频文件
- **空文件/无效视频**: 0 字节文件或非视频文件会抛出明确错误（`ValueError` / `RuntimeError`）
- **关键帧采样间隔**: 默认 2 秒，仅用于规则引擎降级路径，可通过 `DetectorConfig.frame_interval` 调整
